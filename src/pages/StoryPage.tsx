...
export default function StoryPage() {
  ...
  const handleSendMessage = async (content: string) => {
    ...
    try {
      ...
      setState({ ...newState, history: [...newState.history, aiMsg] });
    } catch (error) {
      console.error(error);
      const errorMsg = { 
        id: (Date.now() + 1).toString(), 
          ...
          >
            {isSaved ? (
              ...
              <>
                ...
                <span className="text-sm font-medium">儲存遊戲</span>
              </>
            )}
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 pt-20 scroll-smooth custom-scrollbar">
          <div className="max-w-full md:max-w-7xl mx-auto px-4 md:px-6">
            {state.history.length === 0 ? (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center text-slate-500 mt-32">
                <div className="w-16 h-16 mx-auto border border-slate-800 rounded-full flex items-center justify-center mb-6">
                  <span className="text-2xl">✨</span>
                </div>
                <p className="text-xl font-serif text-slate-300 mb-3">故事即將展開...</p>
                <p className="text-sm text-slate-500">請在下方輸入你的第一個行動，例如：「環顧四周」或「推開眼前那扇沉重的木門」。</p>
              </motion.div>
            ) : (
            ...
            {isLoading && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-3 text-indigo-400 text-sm mt-6 mb-4">
                <Loader2 className="w-5 h-5 animate-spin" />
                <span className="font-serif italic tracking-wide">命運正在編織新的篇章...</span>
              </motion.div>
            )}
